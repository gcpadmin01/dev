...
var initMap = function() {
    var objMrk = null,position= null,location=null,icon_pos,temp = null;
    ...
    elementMaps   = ['maps-litle','maps-expanded','maps'];


    for(var i in elementMaps){
        var rowMap = elementMaps[i];
        var map;
        maps[rowMap]  = {};
        
        map = new google.maps.Map(document.getElementById(rowMap), {
            zoom: 15,
        ...
        objMarkerResort = Object.assign({},objMarker);
        ...
        markerResort = new google.maps.Marker(objMarkerResort);
        maps[rowMap]['mainResortMarker'] = markerResort;
        ...
        for(var row of resorts){
            ...
            objMrk   = Object.assign({},objMarker);
            ...
            temp = new google.maps.Marker(objMrk);
            temp.addListener('click',setInfo);
            maps[rowMap]['marker'].push(temp);
        }
    ...
    infowindow.addListener("closeclick",function(){
        var markers = maps[marker.getMap().getDiv().getAttribute('id')]['marker'];
        var mrkResort = maps[marker.getMap().getDiv().getAttribute('id')]['mainResortMarker'];

        if(!exlusionMarker(marker.type_icon) && directionsRenderer.getMap()!= null){   
            backMarker(mrkResort);
        }
        directionsRenderer.setMap(null);
        visibilityMarkers(markers,marker.getMap().getZoom()>=ZOOM_START,true);
    });
}
...
$('body').on('click','.position-map',function(event) {
    ...
    var objMap = maps[idMap]['map'];
    var markerSrch = searchMarker(idMap,point);

    if(marker != null && objMap.getZoom()>=ZOOM_START){
...
var directionRoute = function(){
    ...
    directionsService.route({
            origin:  (maps[mapName]['mainResortMarker']).getPosition(),
        ...
        function(response, status) {
            if (status === google.maps.DirectionsStatus.OK) {
                ...
                visibilityMarkers(maps[mapName]['marker'],false);
...
var setInfo = function(){
    ...
    marker = this;        
    if(exlusionMarker(this.type_icon)){
        directionRoute();
    }
}

var searchMarker = function(map,point){
    for(mrk of maps[map]['marker']){
